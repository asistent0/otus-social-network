...
#### Класстеризация и зеркалирование

Установлено 3 ноды и настроена маршрутизация между ними через HAProxy.

Добавлено зеркалирование на 2 очереди для отказоустойчивости. Если одна нода выйдет из строя,
то сообщения не потеряются и продолжат идти на другие ноды.

## Подключение In-MemoryBD Tarantool

Модуль сообщений (`message`) выбран для переноса, так как он требует быстрого доступа к данным в реальном времени.
Это делает его идеальным кандидатом для использования In-Memory СУБД, такой, как Tarantool.

Был добавлен скрипт на языке lua для добавления новой базы в базу Tarantool.
И добавлены функции для добавления сообщений и поиска 100 последних сообщений по id диалога. 

Добавлена команда для добавления существующих данных из базы PostgreSQL в базу Tarantool.

Теперь в эндпоинте добавления сообщения в диалог, оно добавляется сначала в базу PostgreSQL, а потом в базу Tarantool,
Для последующей быстрой выборки.

А в эндпоинте получения сообщений происходит поиск id диалога в базе PostgreSQL, а потом по этому id из базы Tarantool
достаются 100 последних сообщений.

Так как для выборки сообщений используются только 100 последних, то принято решение не хранить все сообщения в
базе Tarantool. Соответственно команда переноса сообщений, переносить только по 100 сообщений по каждому диалогу.
И также добавлена команда, для очистки старых сообщений, которую можно запускать:

Либо в ручную: 

```shell
docker exec social_network_php php bin/console app:cleanup-tarantool-messages
```

Либо по крону в определенное время, например на каждый час:

```shell
0 * * * * docker exec social_network_php php bin/console app:cleanup-tarantool-messages
```

### Проведение тестирования

В папке JMeter3 находятся все отчеты и настройки.
